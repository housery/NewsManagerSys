        ...
        try {
            if (opr.equals("addComment")) {// 添加评论
                ...
                try {
                    ...
                    result = "添加失败，请联系管理员查找错误原因";
                }
                DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:SS");
                out.print("[{\"result\":\""+ result +"\",\"cdate\":\"" + dateFormat.format(comment.getCdate()) + "\"}]");
                ...
                request.getRequestDispatcher("/index.jsp").forward(request,
                        response);
            } else if ("list2".equals(opr)) {// 编辑新闻时对新闻的查找
...
//                response.sendRedirect(contextPath + "/newspages/admin.jsp");
                ...
                for (int i = 0;;){
                    ...
                    newsJSON.append("\"nauther\":\"" + news.getNauthor().replace("\"","&quot;") + "\"}");
                    if ((++i) == list.size()){
                        break;
                    } else {
                        newsJSON.append(",");
                ...
                *//*String newsJSON = JSON.toJSONString(list);
                ...
                String uploadFilePath = getServletContext().getRealPath(
                        "upload");
                String status = "status为空";
                String message = "message为空";
                if (isMultipart) {
